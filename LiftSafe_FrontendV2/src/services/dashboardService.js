...
const apiClient = async (endpoint, options = {}) => {
  ...
  try {
    ...
    if (error.message.includes('Failed to fetch')) {
      throw new Error('No se puede conectar al servidor. Verifica que el backend esté corriendo en http://localhost:8000', { cause: error });
    }
    throw new Error(error.message || 'Error en la petición', { cause: error });
  }
};
...
// ✅ ARREGLADO: Usa las rutas correctas y las que realmente tienen los datos
