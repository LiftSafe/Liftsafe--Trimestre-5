...
export default function DashboardLayout() {
  ...
  // ✅ USA user.rol (lo que devuelve el backend) para generar el menú
  const menuItems = getMenuForRole(user?.rol);
  ...
  const drawer = (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', background: gradients.sidebar }}>
      ...
      <Box sx={{ p: 1.5 }}>
        ...
        <ListItemButton onClick={logout} sx={{ borderRadius: 2, color: '#E87070', py: 0.5 }}>
  ...
  const headerTitle = currentItem?.key === 'dashboard' ? '' : (currentItem?.text || '');
    ...
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: brand.surface }}>
      <AppBar position="fixed" elevation={0} sx={{ bgcolor: '#fff', borderBottom: '1px solid', borderColor: 'divider', width: { md: `calc(100% - ${DRAWER_WIDTH}px)` }, ml: { md: `${DRAWER_WIDTH}px` } }}>
        <Toolbar variant="dense" sx={{ minHeight: 52 }}>
          ...
          <Typography variant="subtitle1" fontWeight={700} sx={{ flexGrow: 1, color: brand.navy }}>
            {headerTitle}
          </Typography>
          ...
          <Menu anchorEl={anchorEl} open={!!anchorEl} onClose={() => setAnchorEl(null)}>
            ...
            <MenuItem disabled><Typography variant="caption" color="text.secondary">{user?.rol}</Typography></MenuItem>
            <Divider />
